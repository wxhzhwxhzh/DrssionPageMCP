...
- 触发信号：浏览器未初始化、会话断开。
- 恢复动作：
  1. 调用 `mcp__mcp_router__connect_or_open_browser`
  2. 调用 `mcp__mcp_router__get_current_tab_info` 验证连接
  3. 重试原动作 1 次
...
- 触发信号：当前无有效标签页。
- 恢复动作：
  1. 调用 `mcp__mcp_router__new_tab` 或 `mcp__mcp_router__get`
...
- 触发信号：XPath/文本定位失败。
- 恢复动作：
  1. `mcp__mcp_router__getSimplifiedDomTree`
